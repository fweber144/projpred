...
plot.vsel <- function(
  ...
  if (isTRUE(deltas)) {
    ...
    if (identical(deltas, "mixed")) {
      if (all(stats != "gmpd")) {
        ylab <- paste0(ylab, " (with intervals showing uncertainty for ",
                       "difference vs. ", baseline_pretty, ")")
      } else if (all(stats == "gmpd")) {
        ylab <- paste0(ylab, " (with intervals showing uncertainty for ",
                       "ratio vs. ", baseline_pretty, ")")
      } else {
        ylab <- paste0(ylab, " (with intervals showing uncertainty for ",
                       "difference [ratio for GMPD] vs. ", baseline_pretty, ")")
      }
    }
  }
