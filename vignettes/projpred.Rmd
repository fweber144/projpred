...
Since we will be using the following plot only to determine `nterms_max` for subsequent `cv_varsel()` runs, we can omit the predictor ranking from the plot by setting `ranking_nterms_max` to `NA` (which requires `size_position = "primary_x_bottom"`, which we set here via its global option, just like arguments `text_angle` and `show_cv_proportions` of `plot.vsel()`):
```{r plot_vsel_fast}
