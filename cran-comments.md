...
The local check on a Linux system gave the following NOTEs:
    ...
    * checking CRAN incoming feasibility ... [12s/49s] NOTE
    Maintainer: ‘Osvaldo Martin <aloctavodia[ at ]gmail.com>’
    ...
    * checking examples ... [28s/28s] NOTE
    Examples with CPU (user + system) or elapsed time > 5s
                          user system elapsed
    as.matrix.projection 9.037  0.415   9.834
...
The inability to verify the current time (see the second NOTE) is also not
related to 'projpred'.

The compilation flag reported in the third NOTE is not set within 'projpred', so
it is probably due to the system that this local check ran on. In CRAN checks,
this NOTE has never appeared yet (but it is shown when checking the previous
CRAN version 2.8.0 locally).
